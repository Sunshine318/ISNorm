...
calc.mRSD<-function(data,batch=data.frame(1:nrow(data)),summary.range=seq(0,100,10),use="mean"){
  ...
  analyte.RSD.summary<-cut.bins(obj=analyte.RSD[,1],bins=seq(0,100,10)) # summary for variables over all batches
  ...
  within.batch.RSD.summary<-cut.bins(na.omit(within.batch.RSD[,1]),bins=seq(0,100,10)) # ,max(within.batch.RSD[,1]
